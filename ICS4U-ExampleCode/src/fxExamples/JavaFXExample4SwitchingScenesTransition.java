 ...
 *
 */
public class JavaFXExample4SwitchingScenesTransition extends Application{
	...
	public void start(Stage primaryStage) throws Exception {
        ...
        //
        Scene scene = layout1.getScene();
        //Set Y of second scene to Height of window
        layout1.translateYProperty().set(scene.getHeight());
        //Add second scene. Now both first and second scene is present
        StackPane parentContainer = new StackPane();
        parentContainer.getChildren().add(layout1);
        
        //Animate Y property
        KeyValue kv = new KeyValue(layout1.translateYProperty(), 0, Interpolator.EASE_IN);
        KeyFrame kf = new KeyFrame(Duration.seconds(3), kv);
        timeline.getKeyFrames().add(kf);    
        //After completing animation, remove first scene
        timeline.setOnFinished(t -> {
        	parentContainer.getChildren().remove(layout1);
        });
